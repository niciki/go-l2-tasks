...
```
Будут выведены все значения, переданные в канал `c`, а потом будут выводиться бесконечно `0`. После получения всех значений, каналы `a` и `b` закроются, но цикл `for` в функции `merge` будет продолжать работать. Это происходит потому, что считывание из закрытого канала возвращает `0`, то `0` будут выводиться бескнечно. Необходимо было проверять при считывании в функции `merge` на то, закрыты ли каналы или нет, чтобы исправить эту ошибку.
